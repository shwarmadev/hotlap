...
function ThreadHeader(
  ...
  // useThreadHeaderOptions owns the git items; the transcript menu is appended
  // after them on iOS. Android renders it through ScreenHeader's menus.
  const nativeRightItems = native.options.unstable_headerRightItems as
    | (() => NativeHeaderItems)
    | undefined;
  const headerOptions =
    transcriptHeaderItems.length === 0
      ? native.options
      : {
          ...native.options,
  ...
  return (
    <>
      <ScreenHeader
        ...
        menus={Platform.OS === "android" ? transcriptMenus : undefined}
        trailing={
...
function ThreadRouteContent(
  ...
  const copyTranscriptFromHeader = useCallback(() => {
  ...
  return (
    <>
      ...
      <ThreadHeader
        ...
        onCopyTranscript={supportsTranscriptExport ? copyTranscriptFromHeader : undefined}
      />
