...
import type {
  ScreenHeaderAction,
  ScreenHeaderMenu,
} from "../../components/ScreenHeader.types";
...
function ThreadHeader(
  props: Parameters<typeof useThreadHeaderOptions>[0] & {
    ...
    readonly supportsTranscriptExport: boolean;
  ...
  const { supportsTranscriptExport, onCopyTranscript } = props;
  const nativeHeader = useThreadHeaderOptions(props);
  const transcriptHeaderItems = useMemo<ReadonlyArray<Record<string, unknown>>>(
    () =>
      supportsTranscriptExport
        ...
        : [],
    [onCopyTranscript, supportsTranscriptExport],
  );
  // The native (iOS) header owns its right items, so the transcript menu is
  // appended there; Android renders it as an in-flow header menu below.
  const nativeRightItems = nativeHeader.options.unstable_headerRightItems;
  const native =
    nativeRightItems !== undefined && transcriptHeaderItems.length > 0
      ? {
          ...nativeHeader,
          options: {
            ...nativeHeader.options,
            unstable_headerRightItems: (...args: Parameters<typeof nativeRightItems>) => [
              ...nativeRightItems(...args),
              ...transcriptHeaderItems,
            ],
          },
        }
      : nativeHeader;
  const transcriptMenus = useMemo<ReadonlyArray<ScreenHeaderMenu>>(
    () =>
      Platform.OS === "android" && supportsTranscriptExport
        ? [
            {
              title: "Thread actions",
              icon: "ellipsis",
  ...
  return (
    <>
      <ScreenHeader
        ...
        optionsVersion={[props.gitControls.projectScripts, supportsTranscriptExport]}
        trailing={
  ...
  return (
    <>
      ...
      <ThreadHeader
        ...
        supportsTranscriptExport={supportsTranscriptExport}
