...
import { useThreadGitCenterHeaderItems, useThreadGitRightHeaderItems } from "./ThreadGitControls";
import {
  StackActions,
  useFocusEffect,
  useNavigation,
...
function ThreadHeader(
  ...
  const { onCopyTranscript, supportsTranscriptExport } = props;
  ...
  // Android renders header menus in-flow; iOS appends the transcript menu to
  // the native right items (ScreenHeader skips `menus` once those are set).
  const androidTranscriptMenus = useMemo<ReadonlyArray<ScreenHeaderMenu>>(
    () =>
  ...
  const headerOptions =
    Platform.OS === "ios" && supportsTranscriptExport
      ? {
          ...native.options,
...
function ThreadRouteContent(
  ...
  const handleCopyTranscriptPress = useCallback(() => {
