...
> {
  ...
  // Build an explicit copy of process.env minus either data-home alias (dev-runner
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "T3CODE_HOME" || key === "HOTLAP_HOME") continue;
  ...
  const baseConfig = {
    ...
    // env is already a complete process.env minus both data-home aliases; pass it
