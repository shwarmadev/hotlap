...
describe("DesktopBackendConfiguration", () => {
  ...
  it.effect("resolvePrimary captures backend output in dev so child logs can be persisted", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const baseDirIndex = config.args.indexOf("--base-dir");
        assert.isAtLeast(baseDirIndex, 0);
        assert.equal(config.args[baseDirIndex + 1], baseDir);
      }).pipe(
        Effect.provide(
          DesktopBackendConfiguration.layer.pipe(
            Layer.provideMerge(serverExposureLayer),
  ...
  it.effect("resolveWsl preserves existing WSLENV entries when forwarding backend secrets", () =>
    Effect.gen(function* () {
      ...
      const previousHotlapHome = process.env.HOTLAP_HOME;
      ...
      try {
        ...
        process.env.HOTLAP_HOME = "C:\\Wrong Windows Profile";
        ...
        yield* Effect.gen(function* () {
          ...
          assert.isUndefined(config.env.HOTLAP_HOME);
        ...
        restoreEnv("HOTLAP_HOME", previousHotlapHome);
