...
const BOOT_SERVICE_NAME = "hotlap";
const BOOT_SERVICE_UNIT_FILE = `${BOOT_SERVICE_NAME}.service`;
// `.service` suffix keeps the label distinct from the desktop app's bundle id
// (ai.usefastlane.code), so launchd and TCC records never collide.
const BOOT_SERVICE_LAUNCHD_LABEL = "ai.usefastlane.code.service";
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    `Environment=HOTLAP_HOME=${quoteSystemdValue(plan.baseDir)}`,
...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    ...
    case "service-disabled":
      return "The service is not enabled to start automatically. Run `hotlap service install` to repair it.";
    case "service-stopped":
      return "The service is not running. Check the service log and `systemctl --user status hotlap.service`, then run `hotlap service install`.";
    case "restart-pending":
      return "A newer version is installed but the service is still running the previous one. Run `hotlap service restart` to switch.";
  }
}
...
export class BootServiceDowngradeRefusedError extends Schema.TaggedError<BootServiceDowngradeRefusedError>()(
  ...
  override get message(): string {
    return `Refusing to replace hotlap@${this.installedVersion} with older hotlap@${this.targetVersion}. Run the command again with --allow-downgrade to continue.`;
  }
}
...
export interface BootServiceStatus {
   ...
   * as its own; `hotlap update --base-dir` learned that by restarting the live
...
export class BootService extends Context.Service<
  BootService,
  {
    readonly install: (options?: {
       ...
       * is running now. `hotlap update` uses this when the user declines the
       * restart, so a later `hotlap service restart` lands on the new version.
       */
...
export const make = Effect.fn("cloud.boot_service.make")(function* (input: {
  ...
  const nodePath = isExecutable ? undefined : host.execPath;
  ...
  // Standalone runtimes host a hidden launcher; npm keeps the standalone
  // script that older Hotlap releases and installed Node services understand.
  const plan: BootServicePlan = {
    program:
      nodePath === undefined
        ? [runtimePaths.entryPath, "__service-launcher"]
        : [nodePath, path.join(path.dirname(runtimePaths.entryPath), "service-launcher.mjs")],
  ...
  const install = Effect.fn("cloud.boot_service.install")(function* (options?: {
    ...
    yield* ensurePinnedRuntimeInstalled({
      ...
      validate: (runtime) =>
        (nodePath === undefined
          ? Effect.void
          : fs.exists(path.join(path.dirname(runtime.entryPath), "service-launcher.mjs")).pipe(
              Effect.mapError(
                (cause) =>
                  new PinnedRuntimeInstallError({
                    step: "checking the npm service launcher",
                    cause,
                  }),
              ),
              Effect.flatMap((exists) =>
                exists
                  ? Effect.void
                  : new PinnedRuntimeInstallError({ step: "checking the npm service launcher" }),
              ),
            )
        ).pipe(
          Effect.andThen(
