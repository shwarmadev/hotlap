...
const version = "1.2.3";
const archiveName = `hotlap-${version}-linux-x64.tar.gz`;
...
it.layer(NodeServices.layer)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("keeps the npm layout required by installed Node launchers", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "hotlap-npm-runtime-" });
      ...
      const installed = yield* ensurePinnedRuntimeInstalled({
        ...
        httpClient: HttpClient.make(() => Effect.die("npm runtime must not download an archive")),
        runner: ProcessRunner.ProcessRunner.of({
          run: (input) =>
            Effect.gen(function* () {
              commands.push(input.command);
              assert.include(input.args, `hotlap@${version}`);
              const staging = input.args[input.args.indexOf("--prefix") + 1]!;
              const entry = path.join(staging, "node_modules", "hotlap", "dist", "bin.mjs");
        ...
        validate: (paths) =>
          fs.exists(paths.entryPath).pipe(
            Effect.flatMap((exists) => (exists ? Effect.void : Effect.die("npm entry missing"))),
            Effect.orDie,
          ),
      });
      assert.equal(
      ...
      assert.deepEqual(pinnedRuntimeCommand(installed), {
        command: "/custom/node",
      ...
      const reused = yield* ensurePinnedRuntimeInstalled({
        ...
        nodePath: "/custom/node",
        runner: ProcessRunner.ProcessRunner.of({
          run: () => Effect.die("complete npm runtime must be reused"),
        }),
        httpClient: HttpClient.make(() => Effect.die("no archive expected")),
      ...
      const error = yield* ensurePinnedRuntimeInstalled({
        ...
        arch: "arm64",
        runner: ProcessRunner.ProcessRunner.of({
          run: () => Effect.die("must preserve existing runtime"),
        }),
        httpClient: HttpClient.make(() => Effect.die("no archive expected")),
        validate: () => Effect.void,
      }).pipe(Effect.flip);
