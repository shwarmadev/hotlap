  ...
  for (let page = 1; page <= RELEASE_INDEX_MAX_PAGES; page += 1) {
    const body = yield* httpClient
      ...
      .pipe(
        ...
        Effect.mapError(() => new CliUpdateError({ reason: "Could not list Hotlap releases." })),
        Effect.timeoutOrElse({
          duration: RELEASE_INDEX_TIMEOUT,
          orElse: () =>
            Effect.fail(new CliUpdateError({ reason: "Timed out listing Hotlap releases." })),
        }),
      );
    const releases = yield* decodeReleaseIndex(body).pipe(
      Effect.mapError(
        () => new CliUpdateError({ reason: "The Hotlap release index had an unexpected shape." }),
      ),
    );
 ...
 * POSIX, a `hotlap.cmd` shim on Windows. `hotlap update` repoints it so the next `t3`
...
export const repointLauncher = Effect.fn("cli.update.repoint_launcher")(function* (input: {
  ...
  if (platform === "win32") {
    ...
    // itself as argv0; the shim is the `hotlap.cmd` next to it only when launched
    ...
    yield* fs
      ...
      .pipe(
        Effect.mapError(
          () =>
            new CliUpdateError({ reason: `Could not rewrite the Hotlap launcher at ${shimPath}.` }),
        ),
      );
  ...
  yield* fs.symlink(input.targetEntryPath, tempLink).pipe(
    ...
    Effect.mapError(
      () =>
        new CliUpdateError({
          reason: `Could not repoint the Hotlap launcher at ${input.launchedAs}.`,
        }),
    ),
  );
 ...
 * only ever sees its own path. Walk PATH for a `hotlap.cmd` whose target is the
...
const updateFlags = {
  ...
  channel: Flag.Literals("channel", CLI_RELEASE_CHANNELS).pipe(
    Flag.withDescription(
      "Release channel to follow. Defaults to the channel this Hotlap was published on.",
    ),
    Flag.optional,
  ),
...
export const updateCommand = Command.make("update", {
  ...
  Command.withDescription(
    "Download a newer Hotlap and switch this machine to it, including the background service when one is installed.",
  ),
...
const belongsToBootService = Effect.fn("cli.update.belongs_to_boot_service")(function* (
  ...
  if (platform === "linux") {
    ...
    return Option.isSome(cgroup) && cgroup.value.includes("/hotlap.service");
  }
  if (platform === "darwin") {
...
export const runUpdate = Effect.fn("cli.update.run")(function* (input: {
  ...
  const nodePath = isExecutable ? undefined : yield* HostProcessExecutablePath;
  const environment = yield* HostProcessEnvironment;
  const httpClient = yield* HttpClient.HttpClient;
  ...
  if (targetChannel === "preview" && currentChannel !== "preview") {
    yield* Console.log(
      [
        `hotlap@${targetVersion} is a preview build.`,
        ...
        `  will have to switch back to ${currentChannel} yourself with \`hotlap update --channel ${currentChannel} --allow-downgrade\`.`,
      ].join("\n"),
    );
  ...
  if (executableCurrent && serviceCurrent) {
    yield* Console.log(
      serviceVersion !== undefined
        ? `Hotlap and its background service are already on ${targetVersion} (${targetChannel}).`
        : `Hotlap is already on ${targetVersion} (${targetChannel}).`,
    );
    return;
  }
  if (!input.allowDowngrade && compareExactServiceVersions(targetVersion, newestInstalled) < 0) {
    return yield* new CliUpdateError({
      reason: `hotlap@${targetVersion} is older than the installed ${newestInstalled}. Pass --allow-downgrade to install it anyway.`,
    });
  }
  ...
  progress.heading(
    ...
    executableCurrent
      ? ""
      : `${currentVersion} → ${targetVersion}${targetChannel === "stable" ? "" : ` (${targetChannel})`}${alreadyOnDisk ? " (already downloaded)" : ""}`,
  );
  ...
  if (serviceInstalled && !serviceCurrent) {
    ...
    if (input.assumeYes) {
      ...
      yield* Console.log(
        "  Not a terminal, so the service keeps running its current version. Rerun with --yes to restart it now, or run `hotlap service restart` later.",
      );
    }
  }
  ...
  const runtime = yield* ensurePinnedRuntimeInstalled({
    ...
    Effect.catchIf(
      ...
      () =>
        Effect.fail(
          new CliUpdateError({
            reason: `No release archive was published for hotlap@${targetVersion}.`,
          }),
        ),
    ),
  );
  ...
  // is rewritten either way so a later `hotlap service restart` lands on the new
  ...
  if (serviceInstalled && !serviceCurrent) {
    ...
    yield* BootService.BootService.pipe(
      ...
      Effect.mapError(
        (error) =>
          new CliUpdateError({
            reason: `hotlap@${targetVersion} is installed but the background service could not be ${restartService ? "updated" : "pointed at it"}: ${error.message}`,
          }),
      ),
    );
  ...
  progress.success(`Installed Hotlap ${targetVersion} at ${runtime.entryPath}`);
  if (Option.isSome(repointed)) {
    yield* Console.log(`  ${repointed.value} now runs ${targetVersion}`);
  } else {
    const quote = (argument: string) =>
      `'${argument.replaceAll("'", platform === "win32" ? "''" : "'\\''")}'`;
    const command = pinnedRuntimeCommand(runtime);
    const invocation = [command.command, ...command.args, "--base-dir", input.baseDir]
      .map(quote)
      .join(" ");
    yield* Console.log(
      platform === "win32" ? `  Run in PowerShell: & ${invocation}` : `  Run: ${invocation}`,
    );
    if (nodePath !== undefined) {
      yield* Console.log(
        `  Existing global npm installation was not changed. To update it: npm install -g hotlap@${targetVersion}`,
      );
    }
  }
  if (serviceUpdated) {
    ...
    yield* Console.log(
      `  Background service still running ${serviceVersion ?? "an unknown version"}. Run \`hotlap service restart\` when you are ready to switch it to ${targetVersion}.`,
    );
