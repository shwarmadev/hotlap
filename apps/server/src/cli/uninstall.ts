...
/**
 * What `hotlap uninstall` would remove for one Hotlap home. Computed before anything
...
export interface UninstallPlan {
  ...
  /** The `hotlap` launcher (symlink or `.cmd` shim) that points into this home's runtime tree. */
...
export const uninstallCommand = Command.make("uninstall", {
  ...
  Command.withDescription(
    "Remove Hotlap from this machine: the background service, the launcher, and every downloaded version. Your projects and threads are kept.",
  ),
...
const runUninstall = Effect.fn("cli.uninstall.run")(function* (input: {
  ...
  if (!plan.service && plan.launcher === undefined && plan.runtimeDir === undefined) {
    yield* Console.log(`Nothing to remove: Hotlap is not installed for ${input.baseDir}.`);
    if (!(yield* HostProcessIsExecutable)) {
      yield* Console.log(
        "  This Hotlap runs from a Node script, so it was installed by npm or built from source. Remove it the same way (`npm uninstall -g hotlap`, or delete the checkout).",
      );
    }
    return;
  }
...
>>>>>>> 3bb06ad910389fcfc886bd5156f41dc73f307f80
  ...
  if (plan.runtimeDir !== undefined) {
    ...
    if (platform === "win32") {
      ...
      yield* Console.log(`${runtimeDir} will be removed once Hotlap exits.`);
    } else {
      yield* fs
  ...
  yield* Console.log("");
  yield* Console.log("Hotlap is uninstalled. Thanks for trying Hotlap.");
});
