...
describe("ssh tunnel scripts", () => {
  it("uses Hotlap archives with a bounded npm fallback for Intel Macs", () => {
    ...
    assert.include(
      script,
      "T3_RELEASE_BASE_URL='https://github.com/shwarmadev/hotlap/releases/download'",
    );
    assert.include(script, 'T3_RUNTIME_DIR="$HOME/.hotlap/runtime/versions/$T3_ARCHIVE_VERSION"');
    assert.include(script, 'T3_ARCHIVE="hotlap-$T3_ARCHIVE_VERSION-$T3_PLATFORM-$T3_ARCH.tar.gz"');
    ...
    assert.include(script, 'if [ "$T3_PLATFORM-$T3_ARCH" = "darwin-x64" ]; then');
    assert.include(script, '"hotlap@$T3_ARCHIVE_VERSION"');
    ...
    // The archive path skips Node discovery; npm and source runners retain it.
...
describe("archive runner script", () => {
  ...
  const makeMirror = Effect.fn("makeMirror")(function* (root: string) {
    ...
    const stem = `hotlap-${archiveVersion}-${platform}-${arch}`;
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const versionsDir = `${home}/.hotlap/runtime/versions`;
