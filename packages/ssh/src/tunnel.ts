...
if [ -n "$T3_NODE_SCRIPT_PATH" ]; then
  # Dev mode: a source checkout on the remote. Node discovery is also used
  # for legacy npm runtimes and Intel Macs, which have no standalone archive.
...
T3_RUNTIME_DIR="$HOME/.hotlap/runtime/versions/$T3_ARCHIVE_VERSION"
T3_NPM_ENTRY="$T3_RUNTIME_DIR/node_modules/hotlap/dist/bin.mjs"
t3_runtime_ready() {
  { [ -x "$T3_RUNTIME_DIR/t3" ] || [ -f "$T3_NPM_ENTRY" ]; } && [ "$(cat "$T3_RUNTIME_DIR/.install-complete" 2>/dev/null)" = "$T3_ARCHIVE_VERSION" ]
}
if ! t3_runtime_ready; then
  mkdir -p "$HOME/.hotlap/runtime/versions"
  ...
  # under it.
  T3_LOCK="$HOME/.hotlap/runtime/versions/.$T3_ARCHIVE_VERSION.install.lock"
...
if ! t3_runtime_ready; then
  ...
  T3_ARCHIVE="hotlap-$T3_ARCHIVE_VERSION-$T3_PLATFORM-$T3_ARCH.tar.gz"
  T3_STAGING="$(mktemp -d "$HOME/.hotlap/runtime/versions/.staging-XXXXXX")"
  trap 'rm -rf "$T3_STAGING" "$T3_LOCK"' EXIT
  # Intel Macs have no standalone archive; retain the published Hotlap npm
  # runtime, including native dependencies, and validate before publication.
  if [ "$T3_PLATFORM-$T3_ARCH" = "darwin-x64" ]; then
    ensure_remote_node_path || { printf 'Hotlap on Intel Macs requires a supported Node version.\\n' >&2; exit 1; }
    if command -v npm >/dev/null 2>&1; then
      npm install --prefix "$T3_STAGING" --no-fund --no-audit "hotlap@$T3_ARCHIVE_VERSION"
    elif command -v pnpm >/dev/null 2>&1; then
      pnpm --package=npm@11 dlx npm install --prefix "$T3_STAGING" --no-fund --no-audit "hotlap@$T3_ARCHIVE_VERSION"
    else
      printf 'Hotlap on Intel Macs requires npm or pnpm to install its runtime.\\n' >&2; exit 1
    fi
    node "$T3_STAGING/node_modules/hotlap/dist/bin.mjs" --version >/dev/null
  else
  t3_fetch() {
...
if [ -f "$T3_NPM_ENTRY" ]; then
  ensure_remote_node_path || { printf 'This Hotlap runtime requires a supported Node version.\\n' >&2; exit 1; }
  exec node "$T3_NPM_ENTRY" "$@"
fi
...
@@T3_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.hotlap/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.hotlap"
...
STATE_DIR="$HOME/.hotlap/ssh-launch/@@T3_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.hotlap"
RUNNER_FILE="$STATE_DIR/run-t3.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
