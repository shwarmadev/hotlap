...
export function cliArchiveStem(version: string, platform: BuildPlatform, arch: BuildArch): string {
  return `hotlap-${version}-${cliArchivePlatformKey(platform, arch)}`;
}
...
/** Stages channel-branded web assets without mutating the shared build output. */
export const stageWebClient = Effect.fn("stageWebClient")(function* (
  ...
  for (const icon of resolveWebIconOverrides(resolveWebAssetBrandForPackageVersion(version), ".")) {
    yield* fs.copyFile(
      path.join(repoRoot, icon.sourceRelativePath),
      path.join(target, icon.targetRelativePath),
    );
  }
...
const buildCliArchive = Effect.fn("buildCliArchive")(function* (input: {
  ...
  yield* stageWebClient(webClient, path.join(contentDir, "client"), input.version);
